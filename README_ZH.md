...
| `resume-team` | "resume my team", "restart my team", "恢复团队", "重新启动" | 跳过 brainstorm，从已有的 `recruitment-plan.md` 重新 spawn，名字和人设重新抽取 |
| `using-agent-teams` | "how do agent teams work", "使用说明", "插件怎么用" | 使用指南、质量标准、角色模板参考 |

`/start-team` 命令会自动编排 `brainstorm-team` → `spawn-team` → `coordinate-team`。当你想跳到特定阶段时使用独立 skill。例如，如果你已有上次运行留下的 `recruitment-plan.md`，只想用新的名字和人设重新 spawn，使用 `resume-team` 代替 `/start-team`。
